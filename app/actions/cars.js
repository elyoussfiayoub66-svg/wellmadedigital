...
export async function submitCarsBooking({ name, phone, fleetSize, lostReservations, role, meetingDate, meetingTime }) {
  const supabase = await createClient();

  // Target User ID for assignments and notifications
  ...
  const { error: apptError } = await supabase.from('appointments').insert({
    ...
    status: 'SCHEDULED',
    title: `Appel Stratégique - ${name} (24/7)`,
  ...
  await supabase.from('notifications').insert({
    ...
    type: 'meeting',
    title: 'Nouvelle Réservation - Catalogue 24/7',
    description: `${name} (${role || 'Propriétaire'}) a réservé un appel le ${meetingDate} à ${meetingTime}. Flotte: ${fleetSize || 'Non spécifiée'}. Perte: ${lostReservations || 'Non spécifiée'}.`,
    ...
    metadata: {
      ...
      role: role,
      lost_reservations: lostReservations
    }
  });
