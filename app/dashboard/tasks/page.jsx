...
export default function TasksPage() {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case 'Assigned': return 'border-blue-200 bg-[#0D1520] text-[#60A5FA] border border-[#15203D]';
      ...
      case 'Completed': return 'border-green-200 bg-[#0D1F0D] text-[#4ADE80] border border-[#1A3D1A]';
      case 'Overdue': return 'border-red-200 bg-red-50 text-red-700';
      default: return 'border-gray-200 bg-brand-bg text-gray-700';
    }
  };
    ...
    <div className="flex flex-col h-[calc(100vh-120px)]">
      <div className="flex items-center justify-between mb-6 shrink-0">
        ...
        <div className="flex items-center gap-4">
          <div className="bg-brand-surface border border-brand-border rounded-lg p-1 ">
            <select 
          ...
          <button 
            ...
            className="flex items-center gap-2 bg-brand-accent text-white px-4 py-2 rounded-lg font-medium hover:opacity-90  transition-opacity disabled:opacity-50"
          >
              ...
              return (
                ...
                >
                  <div className={`px-4 py-3 rounded-t-xl border-t border-x ${getStatusColor(column)} flex items-center justify-between`}>
                    ...
                    <span className="text-xs font-bold bg-brand-surface/50 px-2 py-0.5 rounded-full">
                      {columnTasks.length}
                    </span>
                  </div>
                  
                  <div className="flex-1 bg-brand-surface/50 border border-brand-border rounded-b-xl p-3 flex flex-col gap-3 overflow-y-auto custom-scrollbar">
                    {columnTasks.length === 0 && (
                      <div className="text-xs text-brand-text/30 text-center py-4 border-2 border-dashed border-brand-border rounded-lg">
                        Drop tasks here
                      </div>
                    )}
                    ...
                    {columnTasks.map(task => (
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between mt-3 pt-3 border-t border-brand-border">
                          ...
                          {task.profiles && (
                            <div className="w-6 h-6 rounded-full bg-brand-dark flex items-center justify-center text-[10px] font-bold text-white " title={task.profiles.full_name}>
            ...
            <div className="p-6">
              <form id="task-form" onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <input required type="text" value={formData.title} onChange={e => updateForm('title', e.target.value)} className="w-full bg-brand-bg border border-brand-border rounded-lg px-3 py-2.5 text-sm text-brand-text focus:outline-none focus:ring-1 focus:ring-brand-accent" />
                </div>
                
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-brand-text">Description</label>
                  <textarea rows={3} value={formData.description} onChange={e => updateForm('description', e.target.value)} className="w-full bg-brand-bg border border-brand-border rounded-lg px-3 py-2.5 text-sm text-brand-text focus:outline-none focus:ring-1 focus:ring-brand-accent resize-none"></textarea>
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-sm font-medium text-brand-text">Assign To</label>
                    <select required value={formData.assignee_id} onChange={e => updateForm('assignee_id', e.target.value)} className="w-full bg-brand-bg border border-brand-border rounded-lg px-3 py-2.5 text-sm text-brand-text focus:outline-none focus:ring-1 focus:ring-brand-accent">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input type="date" value={formData.due_date} onChange={e => updateForm('due_date', e.target.value)} className="w-full bg-brand-bg border border-brand-border rounded-lg px-3 py-2.5 text-sm text-brand-text focus:outline-none focus:ring-1 focus:ring-brand-accent" />
                  </div>
                </div>
              </form>
            </div>
